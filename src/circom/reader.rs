...
pub fn load_r1cs<G: Group>(filename: &FileLocation) -> R1CS<G::Scalar> {
    ...
    if filename.ends_with("json") {
        ...
        load_r1cs_from_bin_file::<G>(filename)
    }
}
...
fn load_r1cs_from_bin_file<G: Group>(filename: &Path) -> R1CS<G::Scalar> {
    let reader = OpenOptions::new()
        .read(true)
        .open(filename)
        .expect("unable to open.");
    load_r1cs_from_bin::<G, _>(BufReader::new(reader))
}

/// load r1cs from bin by a reader
pub(crate) fn load_r1cs_from_bin<G: Group, R: Read + Seek>(reader: R) -> R1CS<G::Scalar> {
    let file = from_reader::<G, _>(reader).expect("unable to read.");
